...
- (void)loadSubviews {
    ...
    self.topBgImgV.image = [PCCUtils getPlayerSkinImage:@"plv_skin_topbg"];
    self.topBgImgV.userInteractionEnabled = NO;
    [self addSubview:self.topBgImgV];
    
    self.bottomBgImgV = [[UIImageView alloc] init];
    self.bottomBgImgV.image = [PCCUtils getPlayerSkinImage:@"plv_skin_bottombg"];
...
//    self.backBtn = [self addButton:@"plv_skin_back" selectedImgName:nil title:nil fontSize:17.0 action:@selector(backBtnAction:) inView:self.superview];
    ...
    UIImageView *maskView = [[UIImageView alloc] initWithImage:[PCCUtils getPlayerSkinImage:@"plv_skin_back"]];
...
- (void)modifySwitchScreenBtnState:(BOOL)secondaryViewClosed pptOnSecondaryView:(BOOL)pptOnSecondaryView {
    if (secondaryViewClosed) {
        if (pptOnSecondaryView) {
            [self.closeSecondaryBtn setImage:[PCCUtils getPlayerSkinImage:@"plv_skin_camera"] forState:UIControlStateNormal];
        ...
        [self.closeSecondaryBtn setImage:[PCCUtils getPlayerSkinImage:@"plv_close_secondary"] forState:UIControlStateNormal];
        [self.closeSecondaryBtn setTitle:nil forState:UIControlStateNormal];
    }
}
...
- (UIButton *)addButton:(NSString *)normalImgName selectedImgName:(NSString *)selectedImgName title:(NSString *)title fontSize:(CGFloat)fontSize action:(SEL)action inView:(UIView *)view {
    ...
    if (normalImgName) {
        [btn setImage:[PCCUtils getPlayerSkinImage:normalImgName] forState:UIControlStateNormal];
    }
    if (selectedImgName) {
        [btn setImage:[PCCUtils getPlayerSkinImage:selectedImgName] forState:UIControlStateSelected];
    }
    if (title) {
