...
- (void)setModelDict:(NSDictionary *)modelDict {
    ...
    if (self.mine) {
        ...
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:user[@"pic"]] placeholderImage:[PCCUtils getChatroomImage:@"plv_img_default_avatar"]];
    ...
    switch ([data[@"contentType"] integerValue]) {
        case 1: {
            self.imgView.image = [PCCUtils getChatroomImage:@"plv_kou1_img"];
        } break;
        case 2: {
            self.imgView.image = [PCCUtils getChatroomImage:@"plv_kou2_img"];
        } break;
        default:
            break;
    }
